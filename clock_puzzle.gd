...
func get_time():
	var hour: int = int(ceilf(filter_angle($ShortHand.rotation_degrees.z * -1)) / hour_delta) % 12
	var minute: int = int(ceilf(filter_angle($LongHand.rotation_degrees.z * -1)) / minute_delta) % 60
